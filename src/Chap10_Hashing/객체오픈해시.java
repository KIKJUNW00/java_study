...
class SimpleObject2 {
	...
	//Comparator정의 :  회원번호를 기준으로 비교
	public static final Comparator<SimpleObject2> NO_ORDER = new Comparator<SimpleObject2>() {
	...
	public void scanData(String guide, int i) {
...
class OpenHash {
	...
	static class Bucket { 
		...
		//생성자
		public Bucket(SimpleObject2 data, Status stat) {
			this.data = data;
			this.stat = stat;
		}
	...
	public OpenHash(int size) {
		...
		for (int i = 0; i < size; i++) {
			table[i] = new Bucket(null, Status.EMPTY);
	...
	private Bucket searchNode(SimpleObject2 key, Comparator<? super SimpleObject2> c) {
		...
		for(int i = 0; i < size ; i++) {
			Bucket p = table[hash];
			if (p.getStat() == Status.OCCUPIED && c.compare(p.getData(), key) == 0) {
				return p;
			}
	...
	public int remove(SimpleObject2 key, Comparator<? super SimpleObject2> c) {
		Bucket p = searchNode(key, c); //선택한 버킷
		if (p == null) { //이 키값은 등록되지 않음
			return 1;
		}
