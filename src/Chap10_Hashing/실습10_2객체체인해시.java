...
//--- 생성자(constructor) ---//
	...
	public int hashValue(SimpleObject5 key) {
		return key. keyCode().hashCode() % size;
...
//--- 해시 테이블을 덤프(dump) ---//
	public void dump() {
		for (int i = 0; i < size; i++) {
			...
			while (p != null) {
				System.out.printf("=> %s (%s)	", p.getValue().keyCode(), p.getValue().toString());
				p = p.next;
			}
			System.out.println();
		}
